...
export default function CompetitorWatchContent({
  ...
  // Competitors list (excluding Pandora — has its own dedicated section)
  const competitorBrands = COMPETITOR_BRANDS.filter(b => b.id !== 'pandora');

  // Brand summaries: Pandora first, then competitors
  ...
  // Build deduplicated article rows
  type Row = { article: Article; badges: string[] };
  let allRows: Row[] = [];

  if (activeBrand) {
    // Single-brand view (includes Pandora if selected)
    ...
    // All competitors combined — deduplicated, Pandora excluded (shown separately below)
  ...
  // Build StockSeries — deduplicate by ticker, public brands only
  ...
  // Pandora section — shown when unfiltered OR when Pandora is NOT the active brand filter
  ...
  // Asymmetric brand grid: first brand with content gets featured (2× height)
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-12 md:py-16">
      ...
      {/* ── Section 8: Pandora in the News (unfiltered, or when not filtering by Pandora) ── */}
